    ...
    <template  ref="dropdown" v-slot:list>
      <DropdownMenu>
        <template v-for="child in children">
          ...
          <DropdownItem v-else :key="`drop-parent-${child.name}`" :name="child.name"><common-icon :size="iconSize" :type="child.icon"/><span class="menu-title"> {{child.title}}</span></DropdownItem>
        </template>
      </DropdownMenu>
    </template>
